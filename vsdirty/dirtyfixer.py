...
def bore(
    ...
    def _get_tblr(thick):
        if len(thick) == 2:
            ...
            top = thick[0] if len(thick) > 0 else 0
            bottom = thick[1] if len(thick) > 1 else top
            left = thick[2] if len(thick) > 2 else 0
            right = thick[3] if len(thick) > 3 else left
    ...
    if clip.format.color_family == vs.GRAY:
        ...
        fixY = plane(
            depth(
                core.bore.SinglePlane(
                    upclip, top=yt, bottom=yb, left=yl, right=yr, plane=0
                ),
    ...
    if 1 in planes:
        upclip = clip.resize.Bicubic(
            width=plane(clip, 1).width, height=plane(clip, 1).height, format=vs.YUV444PS
        )
        ...
        if singlePlane:
            fixU = plane(
                depth(
                    core.bore.SinglePlane(
                        upclip, top=ut, bottom=ub, left=ul, right=ur, plane=1
                    ),
                    clip.format.bits_per_sample,
                ),
                1,
    ...
    if 2 in planes:
        upclip = clip.resize.Bicubic(
            width=plane(clip, 2).width, height=plane(clip, 2).height, format=vs.YUV444PS
        )
        upclip = core.std.ShufflePlanes(
            [upclip, depth(clip, 32), depth(clip, 32)],
            planes=[0, 1, 2],
        ...
        if singlePlane:
            fixV = plane(
                depth(
                    core.bore.SinglePlane(
                        upclip, top=vt, bottom=vb, left=vl, right=vr, plane=2
                    ),
                    clip.format.bits_per_sample,
                ),
                2,
    ...
    merged = core.std.ShufflePlanes(
        [fixY, fixU, fixV], planes=[0, 0, 0], colorfamily=vs.YUV
    )
